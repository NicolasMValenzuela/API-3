...
const FormularioVehiculo = () => {
  ...
  // Monitorear cambios en el vehículo del store global
  useEffect(() => {
    if (isEditing && allVehicles && allVehicles.length > 0) {
      const vehicleInStore = allVehicles.find(v => v.idVehiculo === parseInt(id) || v.id === parseInt(id));
      if (vehicleInStore && vehicleInStore.imageUrl) {
        setCurrentImage(vehicleInStore.imageUrl);
      }
    }
  }, [allVehicles, id, isEditing]);
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (imageFile) {
          ...
          // Recargar la imagen actualizada desde el servidor (convierte a base64)
          await dispatch(fetchVehicleImages([vehicleId])).unwrap();
        }
